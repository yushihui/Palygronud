(function () {
  ...
  YamlInline.prototype = {
    ...
    dump: function (value) {
      ...
      if (typeof value == "object") return this.dumpObject(value);
      ...
      if ("" == value) return "";
      if (this.getTimestampRegex().test(value)) return "'" + value + "'";
  ...
  YamlEscaper.prototype = {
    ...
    escapeWithDoubleQuotes: function (value) {
      ...
      for (var i = 0; i < len; ++i) if (i >= maxlen) esc.push("");
      var ret = "";
      ret = value.replace(
        new RegExp(YamlEscaper.escapees.join("|"), "g"),
        function (str) {
          for (var i = 0; i < len; ++i) {
  ...
  YamlEscaper.REGEX_CHARACTER_TO_ESCAPE =
  ...
  YamlDumper.prototype = {
    dump: function (input, inline, indent) {
      ...
      if (inline <= 0 || !this.isObject(input) || this.isEmpty(input)) {
        ...
        var isAHash = !this.arrayEquals(
          this.getKeys(input),
          this.range(0, input.length - 1)
        );
        var willBeInlined;
        for (var key in input) {
          if (input.hasOwnProperty(key)) {
            willBeInlined =
              inline - 1 <= 0 ||
              !this.isObject(input[key]) ||
              this.isEmpty(input[key]);
    ...
    isEmpty: function (input) {
      ...
      if (!ret && typeof input == "object" && !(input instanceof Array)) {
        var propCount = 0;
        for (var key in input) if (input.hasOwnProperty(key)) propCount++;
        ret = !propCount;
      }
      return ret;
    },
